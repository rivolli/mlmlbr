...
compute_landmarking <- function(data, data_preprocessed=NULL) {
    ...
    naive_bayes <- measure_landmark(get_naive_bayes, data_preprocessed)
...
compute_grouped_meta_features <- function(data) {
    ...
    list(simple=simple, statistical=statistical, inftheo=inftheo, landmarking=landmarking)
}


characterization <- function(data, path) {
    classname <- colnames(data)[ncol(data)]
    characfile <- path$get_tempfile(paste(classname, '-characterization', sep=''), '.RData')
    if (file.exists(characfile)) {
        load(characfile)
        return(characresult)
    }
    ...
    #"StMin", "StMax", "StMean", "StSd", "StMinGain", "StRand"
    names(characresult) = c("Cls", "Atr", "Num", "Nom", "Spl", "Dim", "NumRate", "NomRate", "SymMin",
                   "SymMax", "SymMean", "SymSd", "SymSum", "ClMin", "ClMax", "ClMean", "ClSd",
                   "Sks", "SksP", "Kts", "KtsP", "AbsC", "CanC", "Fnd", "ClEnt", "NClEnt",
                   "AtrEnt", "NAtrEnt", "JEnt", "MutInf", "EAttr", "NoiSig", "Nb", "Lda", "Nn");
    save(characresult, file=characfile)
